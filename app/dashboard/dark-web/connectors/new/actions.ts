...
export async function createConnectorAction(
  ...
  try {
    ...
    const config = buildConnectorConfig(formData, connectorType, context)
    ...
    if (error) {
      return { error: error.message || 'Unable to create connector.' }
    }
  } catch (error) {
    return {
      error:
  ...
  redirect('/dashboard/dark-web/connectors?created=1')
}
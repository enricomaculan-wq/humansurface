...
import type { UpdateConnectorFormState } from '../actions'
import { updateConnectorAction } from '../actions'

const initialState: UpdateConnectorFormState = {
  error: null,
}
...
function asNumberString(value: unknown, fallback = '10000') {
  return typeof value === 'number' && Number.isFinite(value)
    ? String(value)
    : fallback
}

export function EditConnectorForm({ connector }: Props) {
  const [connectorType, setConnectorType] = useState<
    'manual_seed' | 'http_feed' | 'csv_upload'
  >(
    connector.connector_type === 'manual_seed' ||
  ...
  const [state, formAction, isPending] = useActionState(
    updateConnectorAction.bind(null, connector.id),
    initialState
  )
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(event) =>
              setConnectorType(
                event.target.value as 'manual_seed' | 'http_feed' | 'csv_upload'
              )
            }
            className="w-full rounded-xl border border-white/10 bg-[#020817] px-3 py-2 text-sm text-white outline-none"
            required
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                name="domain"
                type="text"
                defaultValue={asString(config.domain)}
            ...
            <div>
              ...
              <input
                name="email"
                type="text"
                defaultValue={asString(config.email)}
        ...
        <section className="grid gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div>
            ...
            <input
              name="endpoint_url"
              type="text"
              defaultValue={asString(config.endpoint_url)}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                name="source_name"
                type="text"
                defaultValue={asString(config.source_name, 'HTTP Feed')}
            ...
            <div>
              ...
              <input
                name="source_type"
                type="text"
                defaultValue={asString(config.source_type, 'http_feed')}
          ...
          <div>
            ...
            <input
              name="auth_header"
              type="text"
              defaultValue={asString(config.auth_header)}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select
                name="method"
                defaultValue={asString(config.method, 'GET')}
                className="w-full rounded-xl border border-white/10 bg-[#020817] px-3 py-2 text-sm text-white outline-none"
              >
            ...
            <div>
              ...
              <input
                name="timeout_ms"
                type="number"
                defaultValue={asNumberString(config.timeout_ms, '10000')}
                min={1000}
                max={30000}
                className="w-full rounded-xl border border-white/10 bg-[#020817] px-3 py-2 text-sm text-white outline-none"
              />
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                name="cursor_param"
                type="text"
                defaultValue={asString(config.cursor_param, 'cursor')}
                placeholder="cursor"
                className="w-full rounded-xl border border-white/10 bg-[#020817] px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                name="since_param"
                type="text"
                defaultValue={asString(config.since_param, 'since')}
                placeholder="since"
                className="w-full rounded-xl border border-white/10 bg-[#020817] px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500"
              />
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                name="items_path"
                type="text"
                defaultValue={asString(config.items_path, 'items')}
            ...
            <div>
              ...
              <input
                ...
                type="text"
                defaultValue={asString(config.next_cursor_path, 'next_cursor')}
        ...
        <section className="grid gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              defaultValue={asString(config.source_label, 'CSV Upload')}
              className="w-full rounded-xl border border-white/10 bg-[#020817] px-3 py-2 text-sm text-white outline-none"
            />
          </div>
        </section>
